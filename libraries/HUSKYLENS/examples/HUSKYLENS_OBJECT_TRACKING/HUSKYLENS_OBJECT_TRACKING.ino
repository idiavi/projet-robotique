...
void setup() {
    ...
    huskylens.writeAlgorithm(ALGORITHM_OBJECT_TRACKING); // Activation du suivi d'objet
}
...
void loop() {
    ...
    if (!huskylens.isLearned()) {
        Serial.println(F("Objet non appris !"));
        Robot.Speed(0, 0);
        return;
    }

    if (!huskylens.available()) {
        Serial.println(F("Objet perdu ! Le robot s'arrête."));
        Robot.Speed(0, 0);
        return;
    }
    ...
    Serial.print("Objet détecté à x = ");
    Serial.println(xPos);
